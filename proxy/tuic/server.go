...
func (s *Server) TuicInboundValidator() *account.Validator { return s.validator }
...
// packetConn is implemented by transport/internet/tuic.UDPConn
...
func (s *Server) Process(ctx context.Context, network xnet.Network, conn stat.Connection, dispatcher routing.Dispatcher) error {
	...
	if !dest.IsValid() {
		return errors.New("tuic: missing target from transport")
	}
